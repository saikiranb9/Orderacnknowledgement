...
public class KafkaConfiguration {
	...
	public ConsumerFactory<String, IncomingOrderAck> consumerFactory() {
		Map<String, Object> config = new HashMap<>();
		config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaConfigConstants.BOOTSTRAP_SERVER_URL);
		// config.put(ConsumerConfig.GROUP_ID_CONFIG, "group_orderacknowledgement");
		...
		config.put(JsonDeserializer.VALUE_DEFAULT_TYPE,
				"com.atd.microservices.core.orderacknowledgement.domain.orderack.IncomingOrderAck");
	...
	@Bean
	public ConcurrentKafkaListenerContainerFactory<String, IncomingOrderAck> kafkaListenerContainerFactory() {
		ConcurrentKafkaListenerContainerFactory<String, IncomingOrderAck> factory = new ConcurrentKafkaListenerContainerFactory<>();
		factory.setConsumerFactory(consumerFactory());
		return factory;
	}
	
	@Bean
    public ProducerFactory<String, EDIMapperPayload> producerFactory() {
		Map<String, Object> config = new HashMap<>();
		...
		if (kafkaConfigConstants.SSL_TRUSTSTORE_LOCATION != null
				&& !StringUtils.isBlank(kafkaConfigConstants.SSL_TRUSTSTORE_LOCATION)) {
	...
	@Bean
    public ProducerFactory<String, CompleteCsvAck> csvProducerFactory() {
		Map<String, Object> config = new HashMap<>();
        config.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaConfigConstants.BOOTSTRAP_SERVER_URL);
        config.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        config.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
    ...
    @Bean("ediMapperKafkaTemplate")
    public KafkaTemplate<String, EDIMapperPayload> ediMapperKafkaTemplate() {
        return new KafkaTemplate<>(producerFactory());
    }
    
    @Bean("csvKafkaTemplate")
    public KafkaTemplate<String, CompleteCsvAck> csvKafkaTemplate() {
        return new KafkaTemplate<>(csvProducerFactory());
    }
